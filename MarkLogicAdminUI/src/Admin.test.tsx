...
const mockAllEndpointsWithDetails = (
  ...
  if (
    databasesResponse &&
    databasesResponse['database-default-list']?.['list-items']?.['list-item']
  ) {
    const dbList =
      databasesResponse['database-default-list']['list-items']['list-item'];
    dbList
  ...
  if (
    forestsResponse &&
    forestsResponse['forest-default-list']?.['list-items']?.['list-item']
  ) {
    const forestList =
      forestsResponse['forest-default-list']['list-items']['list-item'];
    forestList
  ...
  if (
    serversResponse &&
    serversResponse['server-default-list']?.['list-items']?.['list-item']
  ) {
    const serverList =
      serversResponse['server-default-list']['list-items']['list-item'];
    serverList
  ...
  if (
    ...
    defaultGroupsResponse['group-default-list']?.['list-items']?.['list-item']
  ) {
    const groupList =
      defaultGroupsResponse['group-default-list']['list-items']['list-item'];
    groupList
  ...
  if (
    ...
    defaultUsersResponse['user-default-list']?.['list-items']?.['list-item']
  ) {
    const userList =
      defaultUsersResponse['user-default-list']['list-items']['list-item'];
    userList
  ...
  if (
    ...
    defaultRolesResponse['role-default-list']?.['list-items']?.['list-item']
  ) {
    const roleList =
      defaultRolesResponse['role-default-list']['list-items']['list-item'];
    roleList
  ...
  if (
    defaultHostsResponse &&
    defaultHostsResponse['host-default-list']?.['list-items']?.['list-item']
  ) {
    const hostList =
      defaultHostsResponse['host-default-list']['list-items']['list-item'];
    hostList
...
describe('Admin', () => {
  ...
  it('handles malformed API response gracefully', async () => {
    ...
    mockAllEndpoints(mockMalformedResponse, mockMalformedResponse, {
      'server-default-list': { 'list-items': { 'list-item': [] } },
    });
  ...
  describe('Forests functionality', () => {
    ...
    it('displays forests list when API call succeeds', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole('heading', { name: 'Forests' }),
        ).toBeInTheDocument();
      });
    ...
    it('displays error message when forests API call fails with HTTP error', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Error:.*Forests: HTTP 404: Not Found/),
        ).toBeInTheDocument();
      });
      ...
      expect(
        screen.queryByRole('heading', { name: 'Forests' }),
      ).not.toBeInTheDocument();
    });
    ...
    it('displays error message when forests API call fails with network error', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Forests: Connection timeout/),
        ).toBeInTheDocument();
      });
    ...
    it('handles empty forests list gracefully', async () => {
      ...
      const forestsList = forestsLists.find(
        (list) => list.style.background === 'rgb(114, 47, 55)', // Burgundy background for forests
      );
    ...
    it('handles malformed forests API response gracefully', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/"some-other-forest-structure"/),
        ).toBeInTheDocument();
      });
  ...
  describe('Servers functionality', () => {
    ...
    it('displays servers list when API call succeeds', async () => {
      ...
      const infrastructureTabButton = screen.getByRole('button', {
        name: /Infrastructure \(Servers & Groups\)/,
      });
    ...
    it('shows servers with hover tooltips', async () => {
      ...
      await waitFor(() => {
        ...
        const tooltipContentDb =
          screen.getByText(/Content Database:/).parentElement;
        expect(tooltipContentDb?.textContent).toContain('Security');
        expect(screen.getByText(/Modules Database:/)).toBeInTheDocument();
        const tooltipModulesDb =
          screen.getByText(/Modules Database:/).parentElement;
    ...
    it('filters out servers without nameref', async () => {
      ...
      const serversList = serversLists.find(
        (list) => list.style.background === 'rgb(52, 73, 94)', // Blue-grey background for servers
      );
    ...
    it('handles malformed servers API response gracefully', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/"some-other-server-structure"/),
        ).toBeInTheDocument();
      });

      // Should not show servers section for malformed data
      expect(
        screen.queryByRole('heading', { name: 'Servers' }),
      ).not.toBeInTheDocument();
    });
    ...
    it('includes servers error in combined error message on API failure', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Error:.*Servers: Servers API error/),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Integration tests', () => {
    it('displays both databases and forests when both API calls succeed', async () => {
      const mockDatabasesResponse = {
        'database-default-list': {
          'list-items': {
            'list-item': [{ nameref: 'Documents' }, { nameref: 'Security' }],
          },
        },
      };
      ...
      mockAllEndpoints(mockDatabasesResponse, mockForestsResponse, {
        'server-default-list': { 'list-items': { 'list-item': [] } },
      });
    ...
    it('handles mixed success/failure scenarios gracefully', async () => {
      ...
      expect(screen.getByText('Documents')).toBeInTheDocument();
      expect(
        screen.getByText(/Forests: Forests service unavailable/),
      ).toBeInTheDocument();
      expect(
        screen.queryByRole('heading', { name: 'Forests' }),
      ).not.toBeInTheDocument();
    ...
    it(
      'complete database details integration test',
      { timeout: 15000 },
      async () => {
        ...
        await waitFor(() => {
          expect(
            screen.getAllByText(/Status: Enabled/).length,
        ...
        await waitFor(() => {
          ...
          expect(screen.getAllByText('Security').length).toBeGreaterThanOrEqual(
            2,
          ); // Database name and security database value
          ...
          expect(
            screen.getByText('Documents-forest-1, Documents-forest-2'),
          ).toBeInTheDocument();
          ...
          const stemmedSearchesElement =
            screen.getByText('Stemmed Searches:').parentElement;
        ...
        await waitFor(() => {
          ...
          expect(
            screen.queryByText('Retired Forests:'),
          ).not.toBeInTheDocument();
        });
        ...
        const forestsSection = screen.getByRole('heading', {
          name: 'Forests',
        }).parentElement;
  ...
  describe('Database details functionality', () => {
    ...
    it('displays database details inline with status, forests count, and security DB', async () => {
      const mockDatabasesResponse = {
        'database-default-list': {
          'list-items': {
            'list-item': [{ nameref: 'Documents', idref: 'doc-123' }],
          },
        },
      };
    ...
    it('handles database details with disabled status and no security DB', async () => {
      const mockDatabasesResponse = {
        'database-default-list': {
          'list-items': {
            'list-item': [{ nameref: 'Test-DB', idref: 'test-123' }],
          },
        },
      };
    ...
    it('shows loading state for database details while fetching', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Loading database details...'),
        ).toBeInTheDocument();
      });
    });
    ...
    it('handles database details fetch errors gracefully', async () => {
      ...
      (fetch as any).mockImplementationOnce(() =>
        Promise.reject(new Error('Database details fetch failed')),
      );
    ...
    it('shows detailed hover tooltip when hovering over database name', async () => {
      ...
      await waitFor(() => {
        ...
        const securityField =
          screen.getByText('Security Database:').parentElement;
  ...
  describe('Raw Database Details section', () => {
    it('displays raw database details JSON when details are loaded', async () => {
      ...
      expect(
        screen.getByText('View Raw Database Details JSON'),
      ).toBeInTheDocument();

      // Verify the JSON content contains the expected database details
      const detailsSection = screen.getByText(
        'View Raw Database Details JSON',
      ).parentElement;
    ...
    it('does not display raw database details section when no details are loaded', async () => {
      ...
      expect(
        screen.queryByText('View Raw Database Details JSON'),
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe('Forest details functionality', () => {
    ...
    it('displays enhanced forest details in hover tooltip', async () => {
      const mockForestsResponse = {
        'forest-default-list': {
          'list-items': {
            'list-item': [{ nameref: 'Documents', idref: '123' }],
          },
        },
      };
