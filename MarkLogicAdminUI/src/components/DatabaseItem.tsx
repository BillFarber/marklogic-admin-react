...
export const DatabaseItem = React.memo(function DatabaseItem({
  database,
  databaseDetails,
  hoveredDatabase,
  setHoveredDatabase,
    ...
    >
      ...
      {details && (
        <div style={{ fontSize: '0.9em', color: '#ccc', marginTop: '4px' }}>
          Status: {details.enabled ? 'Enabled' : 'Disabled'} | Forests:{' '}
          {Array.isArray(details.forest) ? details.forest.length : 0} | Security
          DB: {details['security-database'] || 'N/A'}
        </div>
      )}
      ...
      {isHovered && details && (
        ...
        >
          ...
          <div>
            <strong>Database ID:</strong> {database.idref}
          </div>
          <div>
            <strong>Name:</strong>{' '}
            {details['database-name'] || database.nameref}
          </div>
          <div>
            <strong>Enabled:</strong> {details.enabled ? 'Yes' : 'No'}
          </div>
          <div>
          ...
          <div>
            ...
            {details['security-database'] || 'N/A'}
          </div>
          <div>
            <strong>Schema Database:</strong>{' '}
            {details['schema-database'] || 'N/A'}
          </div>
          <div>
            <strong>Triggers Database:</strong>{' '}
            {details['triggers-database'] || 'N/A'}
          </div>
          <div>
            <strong>Forests:</strong>{' '}
            {Array.isArray(details.forest) ? details.forest.join(', ') : 'None'}
          </div>
          <div>
            <strong>Data Encryption:</strong>{' '}
            {details['data-encryption'] || 'off'}
          </div>
          <div>
            <strong>Stemmed Searches:</strong>{' '}
            {details['stemmed-searches'] ? 'Enabled' : 'Disabled'}
          </div>
          <div>
            <strong>Word Searches:</strong>{' '}
            {details['word-searches'] ? 'Enabled' : 'Disabled'}
          </div>
          {details['retired-forest-count'] > 0 && (
            <div>
              <strong>Retired Forests:</strong>{' '}
              {details['retired-forest-count']}
            </div>
          )}
        </div>
      )}
