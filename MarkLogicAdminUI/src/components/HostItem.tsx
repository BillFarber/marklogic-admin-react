...
export const HostItem = React.memo(function HostItem({
  host,
  hostDetails,
  hoveredHost,
  setHoveredHost,
      ...
      {isHovered && (
        ...
        >
          ...
          <div style={{ fontSize: '0.9em' }}>
            <div>
              <strong>ID:</strong> {host.idref}
            </div>
            {host.groupnameref && (
              <div>
                <strong>Group:</strong> {host.groupnameref}
              </div>
            )}
            {hostDetails[host.idref] && (
              <>
                <div>
                  ...
                  {hostDetails[host.idref]['host-name'] || 'N/A'}
                </div>
                <div>
                  <strong>Bind Port:</strong>{' '}
                  {hostDetails[host.idref]['bind-port'] || 'N/A'}
                </div>
                <div>
                  <strong>Foreign Bind Port:</strong>{' '}
                  {hostDetails[host.idref]['foreign-bind-port'] || 'N/A'}
                </div>
                {hostDetails[host.idref]['zone'] && (
                  <div>
                    <strong>Zone:</strong> {hostDetails[host.idref]['zone']}
                  </div>
                )}
                {hostDetails[host.idref]['host-mode'] && (
                  <div>
                    <strong>Mode:</strong>{' '}
                    {hostDetails[host.idref]['host-mode']}
                  </div>
                ...
                {hostDetails[host.idref]['bootstrap-host'] !== undefined && (
                  <div>
                    <strong>Bootstrap Host:</strong>{' '}
                    {hostDetails[host.idref]['bootstrap-host'] ? 'Yes' : 'No'}
                  </div>
                )}
              </>
            )}
