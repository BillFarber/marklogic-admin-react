...
export const HostsSection = React.memo(function HostsSection({
  hosts,
  hostDetails,
  hoveredHost,
  setHoveredHost,
  ...
  if (
    !hosts ||
    !Array.isArray(hosts['host-default-list']?.['list-items']?.['list-item'])
  ) {
    return null;
  }

  const hostsList = hosts['host-default-list']['list-items'][
    'list-item'
