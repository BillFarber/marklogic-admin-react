  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '0.9em', color: '#ccc', marginTop: '4px' }}>
        Type: {server.kindref || 'N/A'} | ID: {server.idref} | Group:{' '}
        {server.groupnameref || 'Default'}
      </div>
      ...
      {isHovered && (
        ...
        >
          ...
          <div>
            <strong>Server ID:</strong> {server.idref}
          </div>
          <div>
            <strong>Name:</strong> {server.nameref}
          </div>
          <div>
            <strong>Type:</strong> {server.kindref || 'N/A'}
          </div>
          <div>
            <strong>Group:</strong> {server.groupnameref || 'Default'}
          </div>
          {server['content-db'] && (
            <div>
            ...
            (() => {
              ...
              return (
                <>
                  ...
                  {details['server-type'] && (
                    <div>
                      <strong>Server Type:</strong> {details['server-type']}
                    </div>
                  )}
                  {details.authentication && (
                    <div>
                      <strong>Authentication:</strong> {details.authentication}
                    </div>
                  )}
                  ...
                  {details.root && (
                    <div>
                      <strong>Root:</strong> {details.root}
                    </div>
                  )}
                  {details.threads && (
                    <div>
                      <strong>Threads:</strong> {details.threads}
                    </div>
                  )}
                  {details['request-timeout'] && (
                    <div>
                      <strong>Request Timeout:</strong>{' '}
                      {details['request-timeout']}s
                    </div>
                  )}
                  {details['session-timeout'] && (
                    <div>
                      <strong>Session Timeout:</strong>{' '}
                      {details['session-timeout']}s
                    </div>
                  )}
                  {details['ssl-hostname'] && (
                    <div>
                      <strong>SSL Hostname:</strong> {details['ssl-hostname']}
                    </div>
                  )}
                  {details['default-user'] && (
                    <div>
                      <strong>Default User:</strong> {details['default-user']}
                    </div>
                  )}
                  {details['public-port'] !== undefined && (
                    <div>
                      <strong>Public Port:</strong>{' '}
                      {details['public-port'] ? 'Yes' : 'No'}
                    </div>
                  )}
                </>
              );
