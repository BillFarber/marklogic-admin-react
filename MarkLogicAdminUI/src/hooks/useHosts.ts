    ...
    useEffect(() => {
        const fetchHosts = async () => {
            try {
                ...
                if (data && Array.isArray(data['host-default-list']?.['list-items']?.['list-item'])) {
                        ...
                        .map(async (host: any) => {
                            try {
                                ...
                                if (detailResponse.ok) {
                                    ...
                                    return { hostKey: host.idref || host.nameref, data: detailData };
                                } else {
                                    console.warn(`Failed to fetch details for host ${host.nameref}: ${detailResponse.status}`);
                                    return null;
                                }
                            } catch (err) {
                                console.warn(`Error fetching details for host ${host.nameref}:`, err);
                                return null;
                            }
                        });
